...
### 运行状态初始化
1. 主会话创建 `.ai-workflow/runs/RUN-{YYYYMMDD-HHMMSS}-feature-flow/`
...
3. tangseng 只输出 RCU-{YYYYMMDD}-{NNN}（需求理解确认单），不得输出 REQ
   ...
   - 主会话追加 `agent_completed` 事件，并将 `status` 更新为 `WAITING_USER`
...
3. **方案选择门控点**：展示 TDR 文档，等待用户逐项选择
   ...
   - 用户确认后，主会话创建 `.ai-workflow/artifacts/architectures/TDR-{YYYYMMDD}-{NNN}.confirmed` 标记文件，追加 `user_confirmed` 事件
...
Phase 3 完成后，主会话必须追加 `agent_completed` 事件，并将 `phase` 更新为 `PLG`、`status` 更新为 `READY`。如 CG 或 CDG 失败，进入 `BLOCKED` 或 `REWORK`，并记录目标 bajie 任务。
...
PLG 通过后，主会话追加 `gate_passed` 事件，并将 `phase` 更新为 `CTG`、`status` 更新为 `READY`。PLG 发现偏差时进入 `REWORK`，记录偏差、目标模块和返工轮次。
...
2. **代码实现问题或 bajie 单测实现问题** → 主会话记录失败用例 → 调用 bajie 修复
3. **测试策略/覆盖盲区问题** → nezha 自行补充测试后重跑
4. bajie 如有修复，返回新变更清单和修复说明
5. 主会话对比修复前后变更清单，计算增量范围
...
测试失败或单测缺口必须进入 `REWORK`，记录失败类型、目标 Agent、返工轮次和是否升级全量测试。nezha 全部通过后，主会话将 `phase` 更新为 `REVIEW`、`status` 更新为 `READY`。
...
评审发现致命/严重问题必须进入 `REWORK`，记录问题等级、目标 Agent、是否行为变更和回退阶段。评审通过后，主会话将 `phase` 更新为 `DONE`、`status` 更新为 `READY`。
...
## 关键约束
- 主会话不得修改 Agent 的输出内容，只做传递、展示、存档、门禁和状态维护
