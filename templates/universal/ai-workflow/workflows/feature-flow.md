...
3. analyst 只输出 RCU-{YYYYMMDD}-{NNN}（需求理解确认单），不得输出 REQ
   - **analyst 输出 RCU 后必须立即停止，不得继续输出 REQ 文档**
...
1. 将用户原始需求 + 已确认 RCU 传递给 analyst Agent
2. analyst 输出 REQ-{YYYYMMDD}-{NNN} 文档
...
4. 将用户选择结果 + REQ 文档传递给 architect Agent
...
1. 调用 qa（补充测试 + 全面回归）
2. 调用 reviewer（代码评审）
3. **存档**：将测试报告写入 `.ai-workflow/artifacts/tests/TEST-{YYYYMMDD}-{NNN}.md`，将评审报告写入 `.ai-workflow/artifacts/reviews/REV-{YYYYMMDD}-{NNN}.md`，更新 `index.md`
